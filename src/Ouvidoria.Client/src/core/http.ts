...
export default class Http {
    ...
    public static async delete<T = unknown>(url: string, data?: unknown, header?: Record<string, string>): Promise<T> {
        return Http.fetch("DELETE", url, data, header);
    }

    public static async get<T = unknown>(url: string, header?: Record<string, string>): Promise<T> {
        return Http.fetch("GET", url, null, header);
    }

    public static async put<T = unknown>(url: string, data?: unknown, header?: Record<string, string>): Promise<T> {
        return Http.fetch("PUT", url, data, header);
    }

    public static async post<T = unknown>(url: string, data?: unknown, header?: Record<string, string>): Promise<T> {
